...
export const useMainNav = (langId = 1, params) => useQuery(['main-nav', langId], async () => {
  ...
  const rootSections = await Promise.all(
    ...
    const children = (forms || []).map(form => ({
      ...
      label: <Link to={`/admin/${menu_section_table_name}/${form.id}`}>{translate[form.lang_values_name]?.value || form.lang_values_name}</Link>
    }))
  ...
  rootItems = rootItems.map(item => ({
    key: item.id,
    label:<Link to={`/admin/metabase/${item.id}`}>{mapTranslates[item.lang_values_name]?.value || item.lang_values_name}</Link>
  }))
