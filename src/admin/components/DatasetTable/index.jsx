...
//   const navigate = useNavigate() //не поддерживается
//   const [ searchParams ] = useSearchParams() не поддерживается
      ...
      if (selection.kod === 'selection_list') {
        buttons.push(
          <MenuOutlined
            onClick={e => {
              e.stopPropagation()
              history.push(`/metaadm/${selectionId}/list/30?id=${item.id}`)
            }}
          />
        )
      }
    ...
    <>
      ...
      >
        ...
        <Col>
          ...
          {!!insert.i1 && <Button
            type='primary'
            onClick={() => history.push(`${location.pathname}/create${location.search}`)}
          >
            Создать запись
          </Button>}
        </Col>
      ...
      <Table
        ...
        onRow={record => ({
          onClick: () => history.push(`${location.pathname}/${record[keylabel]}${location.search}`)
        })}
      />
      {isModal && <DatasetForm
        ...
        onOk={async (values) => {
          ...
          if (data.status === 'error') {
            ...
            history.push(`${location.pathname.split('/').slice(0, -1).join('/')}${location.search}`)
            refetch()
          }
        }}
        onCancel={() => history.push(`${location.pathname.split('/').slice(0, -1).join('/')}${location.search}`)}
      />}
    </>
  )
}
